...
describe VideosController, type: :controller do
  ...
  describe 'GET #new' do
    it 'return new page if user logged in' do
      user = create(:user)
      sign_in_as(user)
      get :new
      expect(response).to have_http_status 200
    ...
    it 'return root page with warning message' do
      get :new
  ...
  describe 'POST #create' do
    ...
    it 'return error message if user not log in' do
    ...
    context 'valid data' do
      ...
      it 'return root page with successfully created video' do
        post :create, params: { video: { title: video.title, url: video.url, description: video.description } }
    ...
    context 'invalid data' do
      ...
      it 'return 422 with title is not present' do
      ...
      it 'return 422 with url is not present' do
      ...
      it 'return 422 with url is invalid' do
        post :create, params: { video: { title: video.title, url: 'abcxyz', description: video.description } }
        expect(response).to have_http_status 422
        expect(flash[:error]).to include "Url is an invalid URL"
      end

      it 'return 422 with url is not Youtube url format' do
        post :create, params: { video: { title: video.title, url: 'http://vimeo.com/123', description: video.description } }
        expect(response).to have_http_status 422
  ...
  describe 'GET #edit' do
    ...
    it 'return edit page if user logged in' do
      sign_in_as(user)
      get :edit, params: { id: video.id }
      expect(response).to have_http_status 200
    end

    it 'return root page with warning message user not log in' do
      get :edit, params: { id: video.id }
      expect(response).to redirect_to root_path
      expect(flash[:alert]).to eq "You must be signed in"
    end

    it 'return root page with error message if you are not video owner' do
  ...
  describe 'PUT #update' do
    ...
    context 'valid data' do
      ...
      it 'return root page with successfully updated video' do
    ...
    context 'invalid data' do
      ...
      it 'return 422 with url is not present' do
        put :update, params: { id: video.id, video: { title: video.title, url: '', description: video.description } }
        expect(response).to have_http_status 422
        expect(flash[:error]).to include "Url can't be blank"
      end

      it 'return 422 with url is invalid' do
        put :update, params: { id: video.id, video: { title: video.title, url: 'abcxyz', description: video.description } }
        expect(response).to have_http_status 422
        expect(flash[:error]).to include "Url is an invalid URL"
      end

      it 'return 422 with url is not Youtube url format' do
        put :update, params: { id: video.id, video: { title: video.title, url: 'http://vimeo.com/123', description: video.description } }
        expect(response).to have_http_status 422
        expect(flash[:error]).to include "Url is not youtube URL"
      end
  ...
  describe 'DELETE #destroy' do
    ...
    it 'return root page with message delete video successfully' do
      sign_in_as(user)
